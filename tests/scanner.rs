...
fn should_scan_syntax_stmt() {
    ...
    assert_eq!(
        scanner.next_token(),
        Ok(Token::StrLit("proto3".to_string()))
    );
...
fn should_scan_import_stmt() {
    ...
    assert_eq!(
        scanner.next_token(),
        Ok(Token::StrLit("other.proto".to_string()))
    );
    assert_eq!(scanner.next_token(), Ok(Token::Semicolon));
    assert_eq!(scanner.next_token(), Ok(Token::EOF));
...
fn should_scan_simple_option() {
    ...
    assert_eq!(
        scanner.next_token(),
        Ok(Token::Ident("java_package".to_string()))
    );
    assert_eq!(scanner.next_token(), Ok(Token::Eq));
    assert_eq!(
        scanner.next_token(),
        Ok(Token::StrLit("com.example.foo".to_string()))
    );
    assert_eq!(scanner.next_token(), Ok(Token::Semicolon));
    assert_eq!(scanner.next_token(), Ok(Token::EOF));
}
...
fn should_scan_enum() {
    ...
    assert_eq!(
        scanner.next_token(),
        Ok(Token::Ident("EnumAllowingAlias".to_string()))
    );
    assert_eq!(scanner.next_token(), Ok(Token::LCurly));
    assert_eq!(scanner.next_token(), Ok(Token::Option));
    assert_eq!(
        scanner.next_token(),
        Ok(Token::Ident("allow_alias".to_string()))
    );
    assert_eq!(scanner.next_token(), Ok(Token::Eq));
    ...
    assert_eq!(
        scanner.next_token(),
        Ok(Token::Ident("UNKNOWN".to_string()))
    );
    assert_eq!(scanner.next_token(), Ok(Token::Eq));
    ...
    assert_eq!(
        scanner.next_token(),
        Ok(Token::Ident("STARTED".to_string()))
    );
    assert_eq!(scanner.next_token(), Ok(Token::Eq));
    ...
    assert_eq!(
        scanner.next_token(),
        Ok(Token::Ident("RUNNING".to_string()))
    );
    assert_eq!(scanner.next_token(), Ok(Token::Eq));
    ...
    assert_eq!(
        scanner.next_token(),
        Ok(Token::Ident("custom_option".to_string()))
    );
    assert_eq!(scanner.next_token(), Ok(Token::RParen));
    assert_eq!(scanner.next_token(), Ok(Token::Eq));
    assert_eq!(
        scanner.next_token(),
        Ok(Token::StrLit("hello world".to_string()))
    );
...
fn should_scan_message() {
    ...
    assert_eq!(
        scanner.next_token(),
        Ok(Token::Ident("my_option".to_string()))
    );
    assert_eq!(scanner.next_token(), Ok(Token::RParen));
    assert_eq!(scanner.next_token(), Ok(Token::Dot));
...
fn should_scan_service_def() {
    ...
    assert_eq!(
        scanner.next_token(),
        Ok(Token::Ident("SearchService".to_string()))
    );
    assert_eq!(scanner.next_token(), Ok(Token::LCurly));
    ...
    assert_eq!(
        scanner.next_token(),
        Ok(Token::Ident("SearchRequest".to_string()))
    );
    assert_eq!(scanner.next_token(), Ok(Token::RParen));
    ...
    assert_eq!(
        scanner.next_token(),
        Ok(Token::Ident("SearchResponse".to_string()))
    );
    assert_eq!(scanner.next_token(), Ok(Token::RParen));
    assert_eq!(scanner.next_token(), Ok(Token::Semicolon));
    assert_eq!(scanner.next_token(), Ok(Token::RCurly));
...
fn should_scan_str_literal() {
    ...
    assert_eq!(
        scanner.next_token(),
        Ok(Token::StrLit("a string{})();,.".to_string()))
    );
}
...
fn should_scan_str_literal_with_escaping() {
    ...
    assert_eq!(
        scanner.next_token(),
        Ok(Token::StrLit("string{with};.\"es(c)aping\".".to_string()))
    );
}
...
fn should_return_err_on_non_closed_str_literal() {
    ...
    assert_eq!(
        scanner.next_token(),
        err("Lexical error: unclosed string literal")
    );
}
